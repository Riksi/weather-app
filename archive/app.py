...
def custom_number_input(label, default='', 
                    name=None, integer=False, 
                    inp_elem=None, err_elem=None):
    if inp_elem is not None:
        inp_elem.empty()
    if err_elem is not None:
        err_elem.empty()
    inp_elem = st if inp_elem is None else inp_elem
    err_elem = st if err_elem is None else err_elem
    name = name or label
    number = inp_elem.text_input(label + ' (optional)', value=default)
    if len(number) == 0:
        number = None
    else:
        try:
            ...
            number = None
            err_elem.error('Please enter a valid value for {}'.format(name))
    return number
...
def choose_from_map(df, 
    lat_default=str(33.627), 
    lon_default=str(-116.159)
):
    ...
    "1. Input the latitute and longitude or click on the map."
    "2. Check 'Find nearest stations' to see the nearest station."
    lat_colm, lon_colm = st.columns(2)
    with lat_colm:
        lat_inp, lat_err = st.empty(), st.empty()
    with lon_colm:
        lon_inp, lon_err = st.empty(), st.empty()

    def lat_lon_input(_lat, _lon):
        with lat_colm:
            lat = custom_number_input('Latitude', 
                        default=_lat, inp_elem=lat_inp, err_elem=lat_err)

        with lon_colm:
            lon = custom_number_input('Longitude', 
                        default=_lon, inp_elem=lon_inp, err_elem=lon_err)
    ...
    loc = {'location': [lat, lon]} if (lat is not None and lon is not None) else {}
    m = folium.Map(zoom_start=12, **loc)

    debug = st.container()
    with debug:
        "Debug:"
    find_container = st.empty()
    map_container = st.empty()
    map_container2 = st.empty()
    with map_container:
        map_data = st_folium(m, width=725, key='map')
    ...
    if map_data['last_clicked'] is not None:
        ...
        'Nearest stations'
        lat_lon_input(_lat=lat, _lon=lon)
    ...
    if lat is not None and lon is not None:
        find = find_container.radio(
            label='Find nearest stations',
            options=['No', 'Yes'],
            index=0,
        ) == 'Yes'

    # with debug:
    #     st.write('Find:', st.session_state.find)
    ...
    if find:
        ...
        m = folium.Map(zoom_start=12, location=[lat, lon])

        for _, row in stations.iterrows():
            location = [row.lat, row.lon]
            m.add_child(folium.Marker(location=location, popup=row.stname))

        with st.spinner('Finding nearest stations...'):
            with map_container2:
                map_data = st_folium(m, width=725, key='map2')
...
def select_weather_station(df):
    with st.container():
        manual = "Input USAF/WBAN manually"
        map_choice = "Choose weather station from map"
